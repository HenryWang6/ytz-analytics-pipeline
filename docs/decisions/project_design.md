...
**Key constraints:**
- AviationStack free tier: 50 req/month, 100 records/req. Extraction runs **3x/week (Mon/Wed/Fri)**, fetching both directions per run with `flight_date` server-side filter. At 2 requests/run × ~13 runs/month = ~26 req/month (48% headroom). The dbt incremental models (3-day and 7-day look-back windows) self-correct flight status transitions across runs — daily extracts are not necessary.
- Record limit: cached data shows YTZ peaks at 101-104 records/direction on busy days (codeshare inflation). The `flight_date` server-side filter keeps results to a single date's flights. **Decision:** cap pagination at 1 page (100 records). On peak days, 1-4 codeshare-duplicate rows may be missed — but all operating-carrier flights (~20-35/day) are captured within the first 100. The airport overview is unaffected; the airline drill-down may miss a few marketing-carrier rows on the busiest days. Acceptable trade-off for a portfolio project.
