...
**Key constraints:**
- AviationStack free tier: 50 req/month, 100 records/req. Extraction runs **3x/week (Mon/Wed/Fri)**, fetching both directions per run. At 2 requests/run × ~13 runs/month = ~26 req/month (48% headroom). The dbt incremental models (3-day and 7-day look-back windows) self-correct flight status transitions across runs — daily extracts are not necessary.
